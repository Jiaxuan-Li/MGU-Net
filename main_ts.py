...
#writer = SummaryWriter('./log2_tsmgunet') 
# logger vis
...
def train(args,train_loader, model, criterion1, criterion2, optimizer,epoch,print_freq=10):
   # set the AverageMeter 
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        writer.add_scalars('Train_loss', {'train_loss1': loss_1.data.item()}, niter)
        writer.add_scalars('Train_loss', {'train_loss2': loss_2.data.item()}, niter)
        writer.add_scalars('Train_loss1', {'train_loss1_ce': loss_1_1.data.item()}, niter)
        writer.add_scalars('Train_loss1', {'train_loss1_dice': loss_1_2.data.item()}, niter)
        writer.add_scalars('Train_loss2', {'train_loss2_ce': loss_2_1.data.item()}, niter)
        writer.add_scalars('Train_loss2', {'train_loss2_dice': loss_2_2.data.item()}, niter)
        """
        print(loss.data.item(), loss_1.data.item(), loss_2.data.item())
...
# evaluation process
def eval(phase, args, eval_data_loader, model, criterion1, criterion2, result_path = None, logger = None):
    # set the AverageMeter 
    batch_time = AverageMeter()
    dice = AverageMeter()
    mpa = AverageMeter()
...
def eval_seg(args, eval_result_path, eval_loader):
    ...
    eval('eval', args, eval_loader, model, criterion1, criterion2,  eval_result_path, logger_eval)
...
def test_seg(args, test_result_path, test_loader):
    ...
    eval('test', args, test_loader, model,  criterion1, criterion2, test_result_path, logger_test)
